function BarGraph(cvas, nBars, barTitles, scaleMin, scaleMax){
    ...
    //canvas dimensions:
    this.width = $('#'+cvas).attr('width');
    this.height = $('#'+cvas).attr('height');
	...
	this.draw = function(frame){
		...
		for(i=0; i<this.nBars; i++){
			...
			barTop = this.height*0.9 - barHeight;
	...
	this.setNewLevels = function(data){
		...
		for(var i=0; i<this.nBars; i++){
			...
			if(this.levels[i] <= 0) this.levels[i] = 0.01;
	...
	this.drawFrame = function(){
		...
		this.context.moveTo(this.width*0.9, this.height*0.9);
		this.context.lineTo(this.width*0.1, this.height*0.9);
		this.context.lineTo(this.width*0.1, this.height*0.1);
		this.context.stroke();

		//draw x-axis labels:
		for(i=0; i<this.nBars; i++){
			this.context.save();
			this.context.translate(this.width*0.1+(i+0.5)*1.05*this.barWidth,this.height*0.92);
